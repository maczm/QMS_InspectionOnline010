...
export default {
  ...
  methods: {
    ...
    getInputStyle(value) {
      const MIN_WIDTH = 200;
      const MAX_WIDTH = 500;
      const CHAR_PADDING = 10;   // el-input__inner 左右 padding
      const BORDER = 2;          // border
      const STATIC_OFFSET = 74;  // 搜索按钮 + 内部间隙，移动端 +62px(相机按钮)

      if (!value) return { width: MIN_WIDTH + "px" };

      let textWidth = 0;
      for (const ch of value) {
        // CJK 字符约 2 倍等宽，ASCII 约 1 倍
    ...
    addProblem() {
      ...
      try {
        if (typeof window.questionAdd !== 'function') {
          loading.close();
          this.$message({
            message: '添加功能不可用',
            type: 'error',
    ...
    addInspection() {
      ...
      try {
        if (typeof window.inspectionAdd !== 'function') {
          loading.close();
          this.$message({
            message: '添加功能不可用',
            type: 'error',
    ...
    async processSelectedFiles(files, questionId) {
      ...
      try {
        ...
        for (const file of files) {
          ...
          // 创建图片对象并添加到列表（不保留 base64/raw file）
          const newImage = {
            name: file.name,
            url: serverUrl,
          };
          problem.imageList.push(newImage);
          loading.setText('图片上传中 (' + problem.imageList.length + '/' + files.length + ')...');
        }
        problem.imgs = problem.imageList.map((f) => f.url).join(",");
        this.syncProblemData();
        loading.close();
